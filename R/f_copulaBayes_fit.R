...
copulaBayesMCMC <- function(y, z, x, num.iterations, verbose) {
  ...
  lambdaz.list <- lapply(mgz.list, function(mg) {
    as.vector(MCMCpack::rdirichlet(1, rep(1, mg$m)))
  })
  lambdax.list <- lapply(mgx.list, function(mg) {
    as.vector(MCMCpack::rdirichlet(1, rep(1, mg$m)))
  })
  ...
  for (i in seq_len(num.iterations)) {
    if (verbose && i %% 500L == 0L) {
      message(
        ...
        i,
        " | alpha = ",
        round(chain[i, col.alpha], 3L),
        " | delta[1] =",
        round(chain[i, col.delta[1L]], 3L),
        " | sigma2 = ",
        round(chain[i, col.sigma2], 3L),
        " | accept rate = ",
        round(n.accepted / i, 2L)
      )
    }
    ...
    chain[i + 1L, col.sa] <- 1 /
      rgamma(1L, shape = 0.501, rate = (alpha.cur^2 + 0.002) / 2)

    for (k in seq_len(K)) {
      chain[i + 1L, col.sb.d[k]] <- 1 /
        rgamma(1L, shape = 0.501, rate = (delta.cur[k]^2 + 0.002) / 2)
    }
    if (L > 0L) {
      for (l in seq_len(L)) {
        chain[i + 1L, col.sb.b[l]] <- 1 /
          rgamma(1L, shape = 0.501, rate = (beta.cur[l]^2 + 0.002) / 2)
      }
    }
  }
