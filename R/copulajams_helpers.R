copulajams_correction_cont <- function(P.all, names.endo.regs, cdf) {
  ...
  P.star <- copula_pstar(P = P.all, cdf = cdf)
  ...
  for (var in factor.vars) {
    ...
    for (lvl in levels.var) {
      ...
      if (nrow(subdat1) <= 3 || !has.variation) {
        next
      } # 3 is number checked by Haschka's repo line 190. #did not find paper backing this up ?
      ...
      P.star <- copula_pstar(P = subdat2, cdf = cdf)
