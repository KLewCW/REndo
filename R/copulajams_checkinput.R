...
# - exogenous: any class but at least 1 continuous (numeric)
# - endogenous: numeric
# - wrongly specified continuous()/discrete() specials
# - warn if any of the endogenous is low-cardinality
#
#' @importFrom Formula as.Formula
#' @importFrom stats terms setNames
checkinput_copulajams_formula_data <- function(formula, data) {
  ...
  if (length(exo.labels) == 0) {
    err.msg <- c(
      err.msg,
      "At least one continuous exogenous regressor (W) is required."
    )
  }
