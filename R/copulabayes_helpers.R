...
copulaBayesConverter <- function(lambda, mg) {
...
copulaBayeslogpost <- function(
  ...
  log.c <- -0.5 *
    log(det(Phi)) -
    0.5 * sum(apply(xi.mat, 1, function(xi) as.numeric(t(xi) %*% A %*% xi)))
...
copulaBayeslogpostparam <- function(
  theta,
  Phi.cur,
  xi.z,
  xi.x,
